function ds = pascal_test_imperfect_exposure(model, testset, year, suffix)
...
try
  ...
  parfor i = 1:num_ids;
    ...
    %% Compute random scale factor with mean zero and standard 
    %% deviation 1 (based on results from als-experiments). Seed random number generator
    %% with test image index before calling randn() to ensure that the scale factors are
    %% repeatable across simulation runs with different models and noise levels. 
    rng(str2num(ids{i}(6:end)));
    scaleFactor = randn();
    im = im*2^scaleFactor;
